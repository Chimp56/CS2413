...
// sets the SID of a student
void Student::set_SID(int x) {
	SID = x;
}

// prints the SID of a student
...
// prints the GPA of a student
void Student::prt_GPA() {
	cout << GPA;
}

// resets a student SID and GPA to -1
void Student::reset() {
	SID = -1;
	GPA = -1;
}
...
// constructor that initializes both SID and GPA to -1
Student::Student() {
	SID = -1;
	GPA = -1;
}
...
// takes the array of students and assigns it to Sooner
void University::set_Stu(Student x[]) {
	for (int i = 0; i < 5; i++) { // loop through the array and assign each student to Sooner
		Sooner[i] = x[i];
	}
}

// prints and returns the mean GPA of the five students held by Sooner[5]
float University::GPA_Mean() {
	float sum = 0;
	for (int i = 0; i < 5; i++) { // sum up all the GPAs
		sum += Sooner[i].get_GPA();
	}
	float mean = sum / 5; // divide the sum by 5 to get the mean
	cout << mean; // print mean
	return mean; // return mean
}

// prints and returns the max GPA of the five students held by Sooner[5]
float University::GPA_Max() {
	float max = Sooner[0].get_GPA(); // set max to the first GPA
	for (int i = 1; i < 5; i++) { // loop through the rest of the GPAs
		if (Sooner[i].get_GPA() > max) { // if the current GPA is greater than max, set max to the current GPA
...
// prints and returns the min GPA of the five students held by Sooner[5]
float University::GPA_Min() {
	float min = Sooner[0].get_GPA(); // set min to the first GPA
	for (int i = 1; i < 5; i++) { // loop through the rest of the GPAs
		if (Sooner[i].get_GPA() < min) { // if the current GPA is less than min, set min to the current GPA
...
// constructor that initializes all student SID and GPA to -1
University::University() {
	for (int i = 0; i < 5; i++) { // loop through Sooner and reset each student
		Sooner[i].reset();
	}
}
// === End of Task 2 ====
...
int main()
{
	...
	for (int i = 0; i < 5; i++) { // loop through x and set each student's SID and GPA
		cin >> sid >> gpa; // take in the SID and GPA
		x[i].set_SID(sid); // set the SID
		x[i].set_GPA(gpa); // set the GPA
	}
	// === End of Task 3 ====
